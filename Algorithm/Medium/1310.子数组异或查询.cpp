...
public:
    vector<int> xorQueries(vector<int>& arr, vector<vector<int>>& queries) {
        vector<int> ans;
        vector<int> xor_front;
        xor_front.push_back(0);
        //xor_front.push_back(0);
        for(int i = 0; i < arr.size() ; i++){
            xor_front.push_back(arr[i] ^ xor_front[i]);
        }
        //ans[i] = xor[left] ^ xor[right + 1]
        for(int i = 0; i < queries.size(); i++){
            ans.push_back(xor_front[queries[i][0]] ^ xor_front[queries[i][1]+1]);
        }
        return ans;
    }
};
